...
#include<dos.h>
using namespace std;
void mainSecPos(int xrad, int midy,int x[60],int y[60])
{
	int i,j=45;
	for(i=360;i>=0;i=i-6)
	{
...
void calcPoints(int radius, int midx, int midy, int x[12], int y[12])
{
	...
	y[11]=midx-x1,y[11]=midy-y1;
	return;
}

int main()
{
	int gd=DETECT,gm,crr,tmp;
	initgraph(&gd,&gm);
	...
	mainSecPos(radius-50,midx,midy,minx,miny);
	mainSecPos(radius-70,midx,midy,secx,secy);
...

	{
		...
		min=data->tm_min%60;
		line(midx,midy,minx[sec],miny[sec]);
		hr=data->tm_hr%60;
		line(midx,midy,hrx[sec],hry[sec]);
		delay(1000)
	}
